var storage='';var isStorageSupport=!0;function openPopup(openBtn,modal,className){var btn=document.querySelector(openBtn)
  ...
  var keyCodeEsc=27;window.addEventListener('keydown',function(e){if(e.keyCode===keyCodeEsc){e.preventDefault();popup.classList.remove(className)
  clearErrors('.js-feedback-form','error')}})}
  openPopup('.js-map-btn','.js-map-container','active')
  openPopup('.js-feedback-btn','.js-feedback-container','active')
  function insertFromLocalStorage(){var name=document.querySelector('.js-feedback-name')
  var email=document.querySelector('.js-feedback-email')
  var text=document.querySelector('.js-feedback-text')
  if(localStorage.getItem('name')){name.value=localStorage.getItem('name');email.value=localStorage.getItem('email');text.focus()}else{name.focus()}}
  function clearErrors(block,className){var block=document.querySelector(block)
  block.classList.remove(className)}
  function validateForm(){var form=document.querySelector('.js-feedback-form')
